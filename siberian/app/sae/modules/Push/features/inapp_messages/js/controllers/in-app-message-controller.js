/**
 * InAppMessage Controller
 *
 * @author Xtraball SAS <dev@xtraball.com>
 * @version 4.17.0
 */
angular.module('starter').controller('InAppMessageController', function ($location, $rootScope, $scope, $stateParams,
                                                                 LinkService, SB, InAppMessage) {
    ...
    InAppMessage.setValueId($stateParams.value_id);

    $scope.loadContent = function (loadMore) {
        var offset = $scope.collection.length;

        InAppMessage.findAll(offset)
            .then(function (data) {
                if (data.collection) {

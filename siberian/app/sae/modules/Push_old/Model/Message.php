...
class Push_Model_Message extends Core_Model_Default
{
    ...
    public function push()
    {
        ...
        foreach ($this->_types as $type => $class_name) {
            ...
            // OneSignal refactoring
            if ($type === 'android') {

                if (in_array($this->getTargetDevices(), ['android', 'all', '', null], false)) {
                    try {

                        // Now we must iterate over every single-app
                        $instance = new Push\Model\Onesignal\Android($this);
                        $instance->push();
    ...
    {
        $devices = [];
        if ($this->getIsStandalone() === true) {
            $_device = (new Push_Model_Android_Device())
                ->find($this->getToken(), "registration_id");
            if ($_device->getPushAlert() === 'enabled') {
                $devices = [$_device];
            }
        } else {
            if ($this->getSendToAll() == 0) {
                $category_message = new Topic_Model_Category_Message();
                $allowed_categories = $category_message->findCategoryByMessageId($this->getId());
            } else {
                $allowed_categories = null;
            }
            ...
            if (Push_Model_Message::hasIndividualPush()) {
                if ($this->getSendToSpecificCustomer() == 1) {

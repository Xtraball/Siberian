...
class LinkStub extends ConstStub
{
    ...
    public function __construct($label, int $line = 0, $href = null)
    {
        ...
        if ($composerRoot = $this->getComposerRoot($href, $this->inVendor)) {
            ...
            $this->attr['ellipsis-tail'] = 1 + ($this->inVendor ? 2 + \strlen(implode_polyfill('', \array_slice(explode(\DIRECTORY_SEPARATOR, substr($href, 1 - $this->attr['ellipsis'])), 0, 2))) : 0);
        } elseif (3 < \count($ellipsis = explode(\DIRECTORY_SEPARATOR, $href))) {
            $this->attr['ellipsis'] = 2 + \strlen(implode_polyfill('', \array_slice($ellipsis, -2)));
            $this->attr['ellipsis-type'] = 'path';

App.config(function ($routeProvider) {
    ...
    $scope.loadContent = function () {
        ...
        McommerceCart.find().success(function (data) {
			...
			if(parseInt($scope.cart.require_datedelivery)==1)
			 {
			 if($scope.cart.delivery_datetime!=null)
			  {$scope.cart.delivery_date = new Date($scope.cart.delivery_datetime);
			  $scope.cart.delivery_time = new Date($scope.cart.delivery_datetime);}
			  else 
			  {var twoHoursIntoFuture = new Date(Math.round((new Date().getTime() + 1000 * 60 * 60 * 2)/(1000*60))*(1000*60));
			   $scope.cart.delivery_date = twoHoursIntoFuture;
			  $scope.cart.delivery_time = twoHoursIntoFuture;}
		     }
    ...
    $scope.updateDeliveryInfos = function () {
        ...
        if (form.$valid) {
            ...
            if(!$scope.is_loading) {
                ...
                var postParameters = {
			        'delivery_date': $scope.cart.delivery_date,
				    'delivery_time': $scope.cart.delivery_time,
				    'delivery_comment' : $scope.cart.delivery_comment,				

App.directive('sbInputNumber', function($window) { 
    return {
        ...
        template: "<div class='input-number' ng-style='styleAlign'><div class='label'>{{ label }}</div> " + "" +
						...
						"<div ng-style='btStyle' class='button toggle-left' ng-click='down2()' ng-show='showten'>-10</div>" +
                        "<div ng-style='labelStyle' class='number'><div>{{ value }}</div></div>" +
						"<div ng-style='margin-right:1px; btStyle' class='button toggle-right' ng-click='up2()' ng-show='showten'>+10</div>" +
        ...
        link: function(scope, element, attrs) {
			...
			scope.showten = parseInt(attrs.showten)==1;
            ...
            scope.down2 = function() {
                if(scope.value>scope.min) {
                    scope.value=scope.value-10;
					 if(scope.value<scope.min)  scope.value=scope.min;
                    scope.callBack();
                }
            };				

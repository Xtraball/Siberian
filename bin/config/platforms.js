...
function downloadPlatform(projectRoot, platform, version, opts) {
    ...
    return Q().then(function() {
        /** @siberian custom call for ssh git repositories. */
        if (opts.platforms[0].indexOf('git@') >= 0) {
            var git_parts = opts.platforms[0].split('#');
            var git_spec = git_parts[0];
            var branchToCheckout = git_parts[1];
            events.emit('log', 'git cloning: ' + git_spec);
            return lazy_load.git_clone(git_spec, branchToCheckout).fail(function(err) {

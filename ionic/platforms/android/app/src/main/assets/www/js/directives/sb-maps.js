        ...
        controller: function($scope, GoogleMaps) {
            ...
            GoogleMaps.addCallback(function() {
                ...
                if ($scope.config.coordinates) {
                    ...
                    if (!$scope.config.coordinates.origin ||
                        ((!$scope.config.coordinates.origin.latitude ||
                            !$scope.config.coordinates.origin.longitude) &&
                            !$scope.config.coordinates.origin.address)) {
                        console.error("An origin (latitude / longitude or address) is required to use Maps");
                        return;
                    }
                    if (!$scope.config.coordinates.destination ||
                        ((!$scope.config.coordinates.destination.latitude ||
                            !$scope.config.coordinates.destination.longitude) &&
                            !$scope.config.coordinates.destination.address)) {
                ...
                if ($scope.config.markers && $scope.config.markers.constructor === Array) {
                    ...
                    // Extend maps to cluster
                    if ($scope.config.cluster !== undefined) {
                        GoogleMaps.addCluster();
                    }
